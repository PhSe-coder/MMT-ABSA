...
class Rule:
      ...
      if item['xpos'] in JJ:
        ...
        if item['deprel'] in MR:
          ...
          if target['xpos'] in NN:
            target_heads = tuple(filter(lambda d: d['head'] == target['id'] and 
  ...
  def R2_2(self, known_targets: Set[str]):
    ...
    doc = self.doc
    for item in doc:
      if item['xpos'] in NN and item['deprel'] == 'nsubj' and item['text'].lower() in known_targets:
        assert item['head'] > 0
        res = filter(lambda d: d['id'] == item['head'] and 
                     d['deprel'] in MR and 
                     d['xpos'] in JJ, doc)
  ...
  def R3_1(self, known_targets: Set[str]):
    ...
    target_dict: Dict[Union[Tuple[int, int], Tuple[int]], str] = {}
    for item in doc:
      if item['deprel'] == 'conj' and item['xpos'] in NN and item['text'].lower() in known_targets:
        assert item['head'] > 0
  ...
  def R3_2(self, known_targets: Set[str]):
    ...
    for item in doc:
      if item['deprel'] == 'obj' and item['text'].lower() in known_targets:
        assert item['head'] > 0
        head = doc[item['head']-1]
        target_heads = tuple(filter(lambda d: d['head'] == head['id'] and 
                          d['deprel'] == 'nsubj' and 
                          d['xpos'] in NN, doc))
        if len(target_heads) > 1:
          logger.debug(f"{' '.join([token['text'] for token in doc])} has MULTIPLE target heads")
  ...
  def R4_2(self, known_opinions: Set[str]):
    ...
    for item in doc:
      if item['deprel'] in MR and item['text'].lower() in known_opinions:
        assert item['head'] > 0
        head = doc[item['head'] - 1]
        opinion_heads = tuple(filter(lambda d: d['head'] == head['id'] and 
                          d['deprel'] in MR and 
                          d['xpos'] in JJ and
                          d['id'] != item['id'], doc))
    ...
    for item in doc:
      if item['xpos'] in JJ:
        ...
        if token in self.pos_words:
          ...
          label = choice(('POS', 'NEG', 'NEU'))
        target_heads = tuple(filter(lambda d: d['head'] == item['id'] and 
                                          d['deprel'] in ['nsubj'] and 
                                          d['xpos'] in NN, doc))
        if len(target_heads) > 1:
          logger.debug(f"{' '.join([token['text'] for token in doc])} has MULTIPLE target heads")
