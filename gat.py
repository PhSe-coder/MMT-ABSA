...
class EGRETLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, edge_dim, use_bias, config_dict=None):
        ...
        self.bn_fc = nn.BatchNorm1d(
            num_features=out_dim, eps=1e-05, momentum=0.1, affine=True,
        ...
        if self.aggregate_edge:
            ...
            self.bn_fc_edge = nn.BatchNorm1d(
        ...
        if self.self_node_transform:
            ...
            self.bn_fc_self = nn.BatchNorm1d(
                num_features=out_dim,
                eps=1e-05,
                momentum=0.1,
                affine=True,
        ...
        if self.transform_edge_for_att_calc:
            ...
            self.bn_fc_edge_for_att_calc = nn.BatchNorm1d(
                num_features=edge_dim,
                eps=1e-05,
                momentum=0.1,
                affine=True,
                track_running_stats=True) if self.use_batch_norm else nn.Identity()
        ...
        if self.edge_dependent_attention:
            if self.transform_edge_for_att_calc:
                z2 = torch.cat([
                    ...
                    self.bn_fc_edge_for_att_calc(self.fc_edge_for_att_calc(edges.data['ex']))
                ],
                               dim=1)
            else:
...
class MultiHeadEGRETLayer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_heads):
            self.heads.append(
                EGRETLayer(in_dim, out_dim, edge_dim, use_bias, config_dict=config_dict)
            )  #in_dim, out_dim, edge_dim, use_bias, config_dict=None
        self.merge = merge
...
class HGAT(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, edge_features, num_heads: int) -> None:
        super(HGAT, self).__init__()
        self.egat = MultiHeadEGRETLayer(in_features,
                                        out_features,
                                        edge_features,
                                        num_heads,
...
config_dict = {
    ...
    'transform_edge_for_att_calc':
    True,  # whether the edge features will be linearly transformed before being used for attention score calculations.
    'apply_attention_on_edge':
    True,  # whether the calculated attention scores will be used for a weighted sum of the edge-features.
    ...
    'edge_dependent_attention':
    True,  # whether edge-features will be used for attention score calculation.
    'self_loop': False,  # or skip connection.
    'self_node_transform':
    True,  # for self_loop (or skip connection), whether we will use a separate linear transformation of the central note
    'activation':
    None  #nn.LeakyReLU(negative_slope=.0) # the only activation/non-linearity in the module. Whether the output (hidden state) will be activated with some non-linearity. Used negative_slope=1 for linear activation
}