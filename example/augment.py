...
def gen_eda(args):
    ...
    output_file = args.output if args.output else join(dirname(args.input), 'eda_' + basename(args.input))
    ...
    with open(args.input, 'r') as dataset, open(output_file, 'w') as writer:
        for _, line in tqdm(enumerate(dataset), total=count):
            ...
            aug_sentences = eda(words, args.alpha_sr, args.alpha_ri, args.alpha_rs, args.alpha_rd, args.num_aug)
            writer.writelines(sentence + '\n' for sentence in aug_sentences)
    print("generated augmented sentences with eda for "
          + args.input + " to " + output_file + " with num_aug=" + str(args.num_aug))
